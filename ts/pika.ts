...
export class Pika extends THREE.Group {
  ...
  private static kDenseRadius = 0.01;
  ...
  private addToPhysics() {
    ...
    const innerBody =
      this.makeRigidBody(innerShell, Pika.kDenseRadius, 0.107/*g*/);
    const outerBody =
      this.makeRigidBody(outerShell, Pika.kRadius, 0.001/*g*/);
  ...
  private makeRigidBody(shape: Ammo.btSphereShape, radius: number, mass: number): Ammo.btRigidBody {
    ...
    ammoTransform.setOrigin(new this.ammo.btVector3(
      this.position.x, this.position.y - radius, this.position.z));
